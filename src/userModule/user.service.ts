...
} from 'src/doctorModule/consultationFormatter.service';
...
export class UserService {
    ...
    async googleOAuthLogin(
        ...
        try {
            ...
            if (!googlePayload) {
                throw new UnauthorizedException(
                    'Google token payload is invalid',
                );
            }
            ...
            if (!googleUserId) {
                throw new UnauthorizedException(
                    'Google token payload is missing user ID',
                );
            }
    ...
    private async generateAndStoreConsultationReport(
        ...
        console.log(`[Consultation Report] Drive URL: ${pdfDriveUrl}`);
