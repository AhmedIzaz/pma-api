...
export class UserController {
    ...
    @Get('/doctors')
    @ApiOperation({
        summary: 'List all doctors and their services for patients',
    })
    async getAllDoctors() {
    ...
    @Get('/appointments')
    @ApiOperation({
        summary: 'List all appointments taken by the logged-in user',
    })
    @UseInterceptors(ClassSerializerInterceptor)
    @SerializeOptions({
    ...
    async getConsultationPrescriptions(@Req() req, @Param('id') id: string) {
        return this.doctorService.getConsultationPrescriptions(
            Number(id),
            req.user.userId,
        );
    }
    ...
    @ApiOperation({
        summary: 'Get conference credentials for a specific consultation',
    })
    @UseInterceptors(ClassSerializerInterceptor)
    @SerializeOptions({
    ...
    async getConferenceCredentials(@Req() req, @Param('id') id: string) {
        const consultation = await this.doctorService.getConsultationById(
            Number(id),
        );
        if (!consultation) {
        ...
        const base64ServerSecret =
            this.configService.get<string>('ZIGO_SERVER_SECRET');
        ...
        const serverSecret = Buffer.from(base64ServerSecret, 'base64').toString(
            'utf-8',
        );
