...
export class PaymentService {
    ...
    private getSslCredentials() {
        const storeIdBase64 = this.configService
            .get<string>('SSL_STORE_ID')
            ?.trim();
        const storePassBase64 = this.configService
            .get<string>('SSL_STORE_NAME')
            ?.trim();
        ...
        const store_id = storeIdBase64
            ? Buffer.from(storeIdBase64, 'base64')
                  .toString('utf-8')
                  ?.replace('\n', '')
            : '';
        const store_passwd = storePassBase64
            ? Buffer.from(storePassBase64, 'base64')
                  .toString('utf-8')
                  ?.replace('\n', '')
            : '';
    ...
    async initiatePayment(dto: InitiatePaymentDto) {
        ...
        const consultation =
            await this.doctorService.getConsultationById(consultationId);
        ...
        const { store_id, store_passwd, baseUrl, domain } =
            this.getSslCredentials();
        ...
        payload.append(
            'success_url',
            `${protocol}://${domain}/${apiBase}/doctor/payments/success`,
        );
        payload.append(
            'fail_url',
            `${protocol}://${domain}/${apiBase}/doctor/payments/fail`,
        );
        payload.append(
            'cancel_url',
            `${protocol}://${domain}/${apiBase}/doctor/payments/cancel`,
        );
        payload.append(
            'ipn_url',
            `${protocol}://${domain}/${apiBase}/doctor/payments/ipn`,
        );
        ...
        try {
            ...
            if (response.data && response.data.status === 'SUCCESS') {
                ...
                this.logger.error(
                    `SSLCommerz init failed: ${JSON.stringify(response.data)}`,
                );
    ...
    async validatePayment(val_id: string, tran_id: string): Promise<boolean> {
        ...
        try {
            ...
            if (payment?.id && payment?.consultationId && payment?.amount) {
                ...
                if (
                    ...
                    this.logger.error(
                        'Amount or currency mismatch during validation',
                    );
    ...
    async handleSuccess(body: any) {
        ...
        this.logger.log(
            `Handling success for tran_id: ${tran_id}, val_id: ${val_id}, ssl_id: ${ssl_id}`,
        );
