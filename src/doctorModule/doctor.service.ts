...
} from './consultationFormatter.service';
...
export class DoctorService {
    ...
    async updateConsultationDuration(
        ...
        const durationMinutes =
            (consultation.durationMinutes || 0) + data.spentMinutes;
    ...
    async getPrescriptionsByConsultationId(
        doctorId: number,
        consultationId: number,
    ) {
        const consultation =
    ...
    async uploadPrescription(
        ...
        setImmediate(async () => {
            try {
                const { txHash, blockchainId } =
                    await this.blockchainService.storeDataOnChain(fileHash);
                console.log('Blockchain store result:', {
                    txHash,
                    blockchainId,
                });
                ...
                console.error(
                    'Failed to store prescription hash on blockchain:',
                    error,
                );
            }
        });
    ...
    async uploadConsultationAudio(
        ...
        setImmediate(async () => {
            try {
                console.log(
                    `[Audio Processing] Started for consultation ${consultationId}. File size: ${file.size} bytes`,
                );
                ...
                console.log(
                    `[Audio Processing] Step 1 completed. Transcript length: ${transcript?.length || 0}`,
                );

                // 2. Format transcript using AI
                console.log(
                    `[Audio Processing] Step 2: Formatting transcript with AI...`,
                );
                const formattedData =
                    await this.consultationFormatterService.formatTranscriptToDraft(
                        transcript,
                    );
                console.log(`[Audio Processing] Step 2 completed.`);
                console.log(
                    `[Audio Processing] Step 2 completed. Formatted data: ${JSON.stringify(formattedData)}`,
                );
                ...
                console.log(
                    `[Audio Processing] Step 3 completed. PDF size: ${pdfBuffer.length} bytes`,
                );

                // 4. Upload PDF to Google Drive
                console.log(
                    `[Audio Processing] Step 4: Uploading PDF to Google Drive...`,
                );
                const pdfFile = {
                ...
                console.log(
                    `[Audio Processing] Step 4 completed. Drive URL: ${pdfDriveUrl}`,
                );
                ...
                console.log(
                    `[Audio Processing] Completed successfully for consultation ${consultationId}.`,
                );
            } catch (error) {
                console.error(
                    `[Audio Processing] Failed for consultation ${consultationId}:`,
                    error,
                );
            }
        });
    ...
    async verifyPrescription(
        ...
        if (prescription.blockchainId) {
            try {
                const record = await this.blockchainService.getRecordFromChain(
                    prescription.blockchainId,
                );
                isBlockchainMatch = this.blockchainService.verifyDataHash(
                    fileHash,
                    record.dataHash,
                );
            } catch (error) {
