...
export class ConsultationFormatterService {
    ...
    `;
        ...
        try {
            ...
            const response =
                await this.symptomAiService.getGenericAiResponse(prompt);
            this.logger.log('Transcript formatted successfully.');
            return JSON.parse(response);
        } catch (error) {
            this.logger.error(
                'Failed to format transcript via AI. Falling back to default data.',
                error,
            );
            return {
                patientSymptoms:
                    'Patient complains of persistent dry cough and light fever for 3 days.',
                doctorDiagnosis:
                    'Mild Upper Respiratory Tract Infection (URTI).',
                prescribedMedications: [
                    'Paracetamol 500mg - 1 tablet 3 times a day after meal',
    ...
    /**
     * Formats pre-transcribed doctor/patient speech (from mobile client) into
    ...
    `;
        ...
        try {
            this.logger.log('Formatting client speech using AI...');
            const response =
                await this.symptomAiService.getGenericAiResponse(prompt);
            this.logger.log('Client speech formatted successfully.');
            return JSON.parse(response);
        } catch (error) {
            this.logger.error(
                'Failed to format client speech via AI. Falling back to default data.',
                error,
            );
            return {
                patientSymptoms: patientSpeech,
                doctorDiagnosis: doctorSpeech,
                prescribedMedications: [],
                followUpPlan: 'Follow up as advised by the doctor.',
            };
        }
    }
}
