...
export class DoctorController {
    ...
    @Post('/registration')
    @ApiOperation({
        summary: 'Register a new doctor with profile and service info',
    })
    async registration(
    ...
    @Get('/services')
    @ApiOperation({
        summary: 'List all services/pricing for the logged-in doctor',
    })
    ...
    async createService(@Req() req, @Body() body: CreateDoctorServiceDTO) {
    ...
    @ApiOperation({
        summary:
            'Get doctor dashboard with total earnings and consultation time',
    })
    ...
    @Get('/consultations')
    @ApiOperation({
        summary: 'List all consultations for the logged-in doctor',
    })
    ...
    @Get('/consultation/:id')
    @ApiOperation({
        summary: 'Get consultation details for the logged-in doctor',
    })
    async getConsultation(@Req() req, @Param('id', ParseIntPipe) id: number) {
        return this.doctorService.getConsultationById(id);
    }
    ...
    async createConsultation(@Req() req, @Body() body: CreateConsultationDTO) {
    ...
    @UseInterceptors(FileInterceptor('file'))
    @ApiOperation({
        summary: 'Upload a prescription for a specific consultation',
    })
        ...
        @UploadedFile(
            new ParseFilePipe({
                validators: [
                    ...
                    new FileTypeValidator({
                        fileType:
                            /^(audio\/webm|audio\/ogg|audio\/mp4|audio\/mpeg|video\/webm)$/,
                    }),
                ],
                fileIsRequired: true,
            }),
    ...
    @ApiOperation({
        summary: 'Verify a prescription file against DB and Blockchain',
    })
    ...
    async verifyPrescription(
        ...
        return this.doctorService.verifyPrescription(
